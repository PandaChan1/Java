package com.pandachen.example.config;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.pandachen.example.config.Interceptor.LoginInterceptor;
import com.pandachen.example.config.web.RequestResponseBodyMethodProcessorWrapper;
...
public class APPConfiger implements WebMvcConfigurer, InitializingBean {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new LoginInterceptor(objectMapper))
                //*代表下一级，**代表子孙级
                .addPathPatterns("/api/**")  //后端全部添加拦截，除了登录、注册
                .excludePathPatterns("/api/user/login")
                .excludePathPatterns("/api/user/register")   //首页不登录就允许访问的文章列表
